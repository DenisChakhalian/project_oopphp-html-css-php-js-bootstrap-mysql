...
<div class="boxer">
    ...
    <div class="row alert main">
        <div class="col-12 mt-5">
            <h1 class="display-5">Реєстрація</h1>
        </div>
    ...
    <div class="auth">
        ...
        if (isset($_REQUEST['username'])) {
            ...
            $user_query = "SELECT * FROM users WHERE users.username = '" . $username . "' OR users.email = '" . $email . "' ";
            $user = mysqli_query($con, $user_query);
            $num_rows = mysqli_num_rows($user);
            if ($num_rows) {
                echo "<div class='form'>
                  <h3>Користувач з таким ім'ям або поштою вже існує.</h3><br/>
                  <p class='link'>Спробуйте <a href='registration.php'>зареєструватися</a> ще раз.</p>
                  </div>";
            } else {
                ...
                if ($result) {
                    echo "<div class='form'>
                  <h3>Ви успішно зареєстровані.</h3><br/>
                  <p class='link'>Тепер <a href='login.php'>увійдіть</a> до системи.</p>
                  </div>";
                } else {
                    echo "<div class='form'>
                  <h3>Обов’язкові поля відсутні.</h3><br/>
                  <p class='link'>Спробуйте <a href='registration.php'>зареєструватися</a> ще раз.</p>
                  </div>";
                }
            }
            ...
            <form method="post" name="login">
                <label>
                    <input required type="text" class="form-control" name="username" placeholder="Ім'я" autofocus="true"/>
                </label>
                <label>
                    <input required type="email" class="form-control" name="email" placeholder="Пошта">
                </label>
                <label>
                    <input required type="password" class="form-control" name="password" placeholder="Пароль">
                </label>
                <div class="auth_button">
                    <p class="link"><a href="login.php">Увійти</a></p>
                    <input type="submit" name="submit" value="Зареєструвати" class="btn auth_btn">
                </div>
            </form>
            <?php
        }
